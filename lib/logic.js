...
// Ya NO calculamos automáticamente "los 8 mejores terceros" por puntaje.
// La fórmula real del Mundial 2026 fija qué grupos concretos aportan el
// tercero a cada cruce (ver R32_DEF -> t3). Esta función queda solo
// para mostrar en el resumen de "Clasificados" qué terceros existen,
// pero el cruce real se decide por el campo t3 de cada partido.
export function calcTop8(rg) {
  const tabs = allTabs(rg);
  const gruposTop8 = new Set();
  R32_DEF.forEach((c) => {
    if (c.t3) c.t3.forEach((g) => gruposTop8.add(g));
  });
  return Array.from(gruposTop8);
}
...
// Obtiene el equipo de un slot del cruce. Si es p2=null, usa el campo
// t3 (lista de grupos posibles para el "mejor tercero" de ese cruce) —
// en la fórmula real cada cruce tiene asignado UN grupo concreto de
// origen para su tercero (t3 con un solo elemento), no una lista de
// candidatos a elegir por puntaje.
function getEq(c, tabs) {
  const e1 = tabs[c.p1.g]?.[c.p1.n - 1]?.e || null;
  let e2 = null;
  if (c.p2) {
    e2 = tabs[c.p2.g]?.[c.p2.n - 1]?.e || null;
  } else if (c.t3 && c.t3.length > 0) {
    // Grupo fijo de origen del mejor tercero para este cruce
    const g = c.t3[0];
    e2 = tabs[g]?.[2]?.e || null;
  }
  return { e1, e2 };
